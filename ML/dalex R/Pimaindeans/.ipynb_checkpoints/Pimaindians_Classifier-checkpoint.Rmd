...
## 4-1. データの分割
```{r}
X = data.frame(select(.data = df, !Outcome))
y = data.frame(select(.data = df, Outcome))
```
...
# 重みの初期化方法確認！！
model %>%
  layer_dense(units=32, activation="relu", input_shape = c(dim(X_test)[2]), kernel_initializer = 'glorot_normal') %>%

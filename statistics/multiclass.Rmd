...
sum(pred == iris.test$Species) / length(pred) # 正解率
```
...
```{r}
# explainでpredict
ex_ins_rf <- explain(iris_rf,data=iris.train, y='setosa',type='classification')
predict_data1_explain <- predict(ex_ins_rf, data1)
```

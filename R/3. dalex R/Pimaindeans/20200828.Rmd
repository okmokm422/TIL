...
## 予測
```{r}
clf_lr_pred <- predict(clf_rf, test)
```
...
library(ingredients)
plot(cp_clf_lr_3, color = "_ids_", variables = c("Glucose", "Age", "Pregnancies")) + 
...
library(ingredients)
plot(cp_clf_rf_3, color = "_ids_", variables = c("Glucose", "Age", "Pregnancies")) + 
  scale_color_manual(name = "names:", breaks = 1:2, 
            values = c("#4378bf", "#8bdcbe"), 
            labels = c("kei" , "keiko")) + 
...
```{r}
vip_clf_lr <- DALEX::model_parts(explainer = exp_clf_lr, loss_function =  , type = "variable_importance", N = 20)
vip_clf_lr
```
